...

{
  ...
  compression_for(name, namespace)::
    local backendTrafficPolicy = envoy_gw.gateway.v1alpha1.backendTrafficPolicy;
    backendTrafficPolicy.new(name + '-response-compression')
  ...
  cf_ip_detection_for(name, namespace)::
  ...
  gateway_resources(
    ...
    local items = [gw]
                  ...
                  + (if compression then [root.compression_for(name, namespace)] else [])
                  + (if enableCloudfrontIPDetection then [root.cf_ip_detection_for(name, namespace)] else []);
